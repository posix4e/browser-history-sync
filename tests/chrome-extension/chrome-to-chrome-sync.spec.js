...
test.describe('Chrome to Chrome Real P2P Sync', () => {
  ...
  test('real P2P sync between two Chrome instances', async () => {
    const syncKey = process.env.SYNC_KEY || `test-${Date.now()}`
    
    // Configure both extensions with same sync key
    ...
    const syncData = {
      ...
      syncSuccess: urls1.length === urls2.length,
      timestamp: Date.now()
    }
    
    await fs.mkdir(path.join(__dirname, '../sync-data'), { recursive: true })
    await fs.writeFile(
      path.join(__dirname, '../sync-data/js-to-js-sync.json'),
